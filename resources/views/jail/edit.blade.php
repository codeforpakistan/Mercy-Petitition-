...
@section('content')
    <div class="card bcard mt-2 mt-lg-3">
        ...
        <div class="card-body px-3 pb-1">
            ...
            <form action="{{ route('jail.update', ['id' => $queryData->id]) }}" id="appointmentForm"
                class="js-validation" method="POST" novalidate="novalidate">
                <input type="hidden" id="csrf_token" name="_token" value={{ csrf_token() }}>
                <input type="hidden" id="PSid" name="PSid" value={{ $queryData->id }}>
                @csrf
                @if (count($errors) > 0)
                ...
                <div class="form-group row">
                    <div class="col-sm-3 col-form-label text-sm-right pr-0">
                        <label for="PhysicalStatus" class="mb-0">Jail name</label>
                    </div>
                    <div class="col-sm-5 col-12 tag-input-styler d-inline-flex align-items-center">
                        <input type="text" value="{{ $queryData->jail_name }}"
                ...
                <div class="form-group row">
                    ...
                    <div class="col-sm-5 col-12 tag-input-styler d-inline-flex align-items-center">
                        <select class="form-control" value="Country" name="province_id" id="province_id">
                            <option value="">Please select</option>
                            @foreach($provinces as $province)
                                <option value="{{$province->province_id}}" {{( $province->province_id == $queryData->province_id ) ? 'selected' : '' }}>{{$province->province_name}}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
